...
async function checkContrast(page: Page, name: string, state: string) {
  const violations = await page.evaluate(() => {
    ...
    elements.forEach((el) => {
      ...
      if (hasText && isVisible) {
        ...
        while (bgColorString === "rgba(0, 0, 0, 0)" || bgColorString === "transparent") {
  ...
  expect(violations, `${name} - ${state} state: Found contrast violations: ${JSON.stringify(violations, null, 2)}`).toEqual([]);
}
...
test.describe("AAA Contrast Verification", () => {
  ...
  test("Representative interactive elements should maintain contrast on Hover", async ({ page }) => {
    await page.goto("/");
    // We use JS dispatch because Playwright hover actionability checks are timing out
    await page.evaluate(() => {
      const el = Array.from(document.querySelectorAll('a')).find(a => a.innerText.includes('Easy'));
      if (el) {
  ...
  test("Representative interactive elements should maintain contrast when Active", async ({ page }) => {
    await page.goto("/");
    await page.evaluate(() => {
      const el = Array.from(document.querySelectorAll('a')).find(a => a.innerText.includes('Easy'));
      if (el) {
