...
- [ ] Review and update global CSS and Tailwind config for mobile-first
  defaults.
- [ ] Audit and adjust color palette to ensure WCAG 2.0 AAA contrast compliance.
- [ ] Implement responsive layout for `MainMenu.tsx` using flexbox/grid.
- [ ] Adjust `GameBoard.tsx` to use dynamic sizing for the digit grid based on
  viewport width.
- [ ] Update `Cell.tsx` to ensure touch targets are appropriately sized and
  provide visual feedback on tap.
- [ ] Refactor `HighScoresPage.tsx` to use a mobile-friendly table or list view.
- [ ] Refactor `AboutPage.tsx` for improved readability on narrow screens.
- [ ] Verify responsiveness using browser developer tools across various device
  presets.
