...
describe("GamePage", () => {
  ...
  it("renders the game board for valid difficulty", async () => {
    ...
    mockGeneratePuzzle.mockReturnValue({
      grid: [
        [{ id: '0-0', value: 5, isOn: false }, { id: '0-1', value: 1, isOn: false }],
        [{ id: '1-0', value: 3, isOn: false }, { id: '1-1', value: 2, isOn: false }]
      ],
      rowTargets: [6, 5],
      colTargets: [8, 3],
      rowCurrent: [0, 0],
    ...
    // Check for buttons
    expect(screen.getByRole('button', { name: /Toggle value 5/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Toggle value 3/i })).toBeInTheDocument();
